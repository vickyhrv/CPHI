...
function canViewBudget(row) {
  if (!row) return false;
  return row.role === 'admin' || !!row.can_view_budget;
}
...
function sanitizeUser(row) {
  ...
  return {
    ...
    canViewBudget: canViewBudget(row),
...
function createUser({ username, displayName, password, role = 'user', canViewBudget: budgetFlag = false }) {
  const name = String(username || '').trim().toLowerCase();
  ...
  const info = db.prepare(`
    INSERT INTO users (created_at, updated_at, username, password_hash, display_name, role, enabled, can_view_budget)
    VALUES (@created_at, @updated_at, @username, @password_hash, @display_name, @role, 1, @can_view_budget)
  `).run({
    created_at: now,
    updated_at: now,
    username: name,
    ...
    can_view_budget: isAdmin || budgetFlag ? 1 : 0,
  });
  return sanitizeUser(getById(info.lastInsertRowid));
}

function updateUser(id, { displayName, role, enabled, canViewBudget: budgetFlag }) {
  ...
  let nextBudget = existing.can_view_budget ? 1 : 0;
  if (budgetFlag !== undefined) {
    nextBudget = budgetFlag ? 1 : 0;
  }
  if (nextRole === 'admin') {
    nextBudget = 1;
  }
...
function ensureBootstrapAdmin() {
  ...
  if (hrv && hrv.role !== 'admin') {
    db.prepare(`UPDATE users SET role = 'admin', enabled = 1, can_view_budget = 1, updated_at = ? WHERE id = ?`).run(
